...
namespace Hada {
    ...
    public class ClienteCAD {
		...
		public void inserta_cliente (ClienteEN c) {
			string orden = "insert into clientes values('"+c.nombre+"','"+c.direccion+"','"+c.ciudad+"');" ;
		...
		public void borra_cliente (string n) {
			string orden = "delete from clientes where nombre = '"+n+"'" ;
			int rc = db.exec(orden);
			if (rc != Sqlite.OK) {
